...
export default function VideoCard({
  ...
  const seekToRatio = (ratio: number) => {
    ...
    const nextTime = clamp(ratio, 0, 1) * timelineDuration;
    seekTo(nextTime);
  };

  const startSeekDrag = (event: ReactMouseEvent<HTMLDivElement>) => {
  ...
  return (
    <div
      ...
      onDoubleClick={(event) => {
        const target = event.target as HTMLElement | null;
        if (target?.closest('[data-no-card-drag="true"]')) return;
        event.preventDefault();
        event.stopPropagation();
    ...
    >
        ...
        {!showVideo ? (
          ...
          >
            <img
              ...
              decoding="async"
            />
            <div className="absolute inset-0 bg-linear-to-t from-black/65 via-black/20 to-black/35" />
      ...
      {showVideo && (
        <div
          className="pointer-events-none absolute inset-x-2.5 bottom-3.5 z-20 flex items-center justify-between px-0 py-0 text-white opacity-0 transition-all duration-200 group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100"
        ...
        >
          <div className="flex items-center gap-0.5">
            <Button
              size="icon-xs"
              variant="secondary"
              className="h-6.5 w-6.5 rounded-full border border-white/45 bg-transparent text-white shadow-[0_1px_8px_rgba(0,0,0,0.38)] hover:border-white/80 hover:bg-black/30 dark:hover:bg-white/20"
          ...
          <div className="rounded-full border border-white/40 bg-black/50 px-1.5 py-0.5 text-[10px] text-white backdrop-blur-[1px] dark:bg-background/35">
      ...
      {showTimeline && (
        ...
        >
          ...
          >
            <div
              className="pointer-events-none absolute inset-y-0 z-1 border-x border-primary/95 bg-[#ffffff55]"
              style={{
            ...
            <button
              ...
              className="absolute top-1/2 z-10 h-4 w-[9px] -translate-y-1/2 border border-background/90 bg-primary shadow"
              style={{ left: `calc(${trimRange.start * 100}% - 5px)` }}
            ...
            <button
              ...
              data-trim-handle="true"
              className="absolute top-1/2 z-10 h-4 w-[9px] -translate-y-1/2 border border-background/90 bg-primary shadow"
              style={{ left: `calc(${trimRange.end * 100}% - 5px)` }}
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-10 h-16 bg-linear-to-b from-black/60 via-black/20 to-transparent opacity-0 transition-opacity duration-200 group-hover:opacity-100 group-focus-within:opacity-100" />

      <div
        className={`pointer-events-none absolute left-2 z-20 max-w-[70%] truncate rounded-md border border-white/15 bg-black/45 px-2 py-0.5 text-xs font-medium text-white opacity-0 backdrop-blur-sm transition-opacity duration-200 group-hover:opacity-100 group-focus-within:opacity-100 ${hasTrimChanges ? "top-10" : "top-2"}`}
      >
